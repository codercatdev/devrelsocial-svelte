...
export const getReadOnlyClient = async ({ req, res }: { req: any; res: any }) => {
...
export const getReadWriteClient = async ({ req, res }: { req: any; res: any }) => {
	const user = await databases.getDocument<User>(
		'devrel-social-main',
		'users',
		await getUserIdFromJwt({ req, res })
...
const oa = new OAuth(
	...
	'1.0A',
	`${
		process.env.NODE_ENV === 'development' ? 'http://localhost:3000' : 'https://devrelsocial'
...
export const getOAuthRequestToken = async (req: any, res: any) => {
	...
	oa.getOAuthRequestToken(async function (error, oAuthToken, oAuthTokenSecret, results) {
		...
		try {
			await databases.updateDocument('devrel-social-main', 'users', parsedJwt.userId, {
				oAuthToken,
				redirect: req.headers.referer
			});
		} catch (error) {
			try {
				await databases.createDocument('devrel-social-main', 'users', parsedJwt.userId, {
					oAuthToken,
					redirect: req.headers.referer
				});
			} catch (error) {
	...
	oa.getOAuthRequestToken(function (error, oAuthToken, oAuthTokenSecret, results) {
		...
		oa.getOAuthAccessToken(
			...
			async (error, oAuthAccessToken, oAuthAccessTokenSecret, results) => {
				...
				try {
					const users = await databases.listDocuments<any>('devrel-social-main', 'users', [
					...
					await databases.updateDocument('devrel-social-main', 'users', original?.$id, {
						oAuthAccessToken,
